...
try:
    client.collections["DSE Static-Site"].delete()
except ObjectNotFound:
    pass

current_schema = {
    "name": "DSE Static-Site",
    "fields": [
        {"name": "id", "type": "string"},
...
for x in tqdm(files, total=len(files)):
    ...
    for v in facs:
        ...
        pages += 1
        cfts_record = {
            "project": "DSE Static-Site",
        }
        record = {}
        record["id"] = os.path.split(x)[-1].replace(".xml", f".html?tab={str(pages)}")
        cfts_record["id"] = record["id"]
        cfts_record["resolver"] = f"https://github.com/acdh-oeaw/dse-static/{record['id']}"
...
print(make_index)
print("done with indexing DSE Static-Site")

make_index = CFTS_COLLECTION.documents.import_(cfts_records, {"action": "upsert"})
print(make_index)
print("done with cfts-index DSE Static-Site")
